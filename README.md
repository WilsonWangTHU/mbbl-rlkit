...
This is the [repo](https://github.com/vitchyr/rlkit) by Vitchyr Pong.
We updated the environments so that we can perform benchmarking as part of the [Model Based Reinforcement Learning Benchmarking Library (MBBL)](https://github.com/WilsonWangTHU/mbbl). Please refer to the project page for more information.

## Benchmarking

### Installation
Please follow the standard installation in the below original readme.
Then please go to [MBBL](https://github.com/WilsonWangTHU/mbbl) to install the mbbl package for the environments.

### run the code
We only support the running of SAC and TD3.
First copy the scripts under the root directory.
```
cp exp_scripts/*.sh ./
```
